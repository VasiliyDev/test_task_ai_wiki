...
const PROMPT_ORDER = ["classify.md", "baseline.md", "generate.md", "generate-rigid.md", "verify.md"];
...
function buildPromptsPage() {
  ...
  const body =
    ...
    `<p class="meta">Цепочка: классификатор → генерация в трёх вариантах (RAW / SOFT / RIGID) → слепой критик. ` +
...
function injectNav() {
  ...
  const nav =
    `<body><main>\n<p class="meta"><a href="./prompts/index.html">↳ Промты пайплайна</a> · ` +
...
## Вывод по 10 запросам
- **RAW всегда последний** (10/10). Даже когда голый запрос оборачивается в нормальный вопрос
  и модель выдаёт полноценный ответ — он всё равно проигрывает обоим промтам по полноте,
  извлекаемости и добавочной ценности. Промт реально нужен, гипотеза подтверждается.
- **SOFT и RIGID идут вровень (5/5).** RIGID берёт верх там, где много твёрдых фактов:
  спортсмен с карьерой и статистикой (Роналду, Грета), гаджеты с характеристиками (iPhone,
  Steam Deck, Tesla) — таблицы и FAQ дают полноту и цитируемость.
- **SOFT выигрывает на «образных» темах** (Дудь, Лебедев, Бузова, Dyson, PS5): живее читается
  и несёт меньший риск шаблонности (нейрослоп) в поиске.
- **Практический вывод:** один универсальный промт не оптимален. Рабочая гипотеза — выбирать
  ветку по типу запроса (или гибрид: подача SOFT + структурные блоки для фактологичных тем).
